...
def run(
  ...
  # EXIMBANK
  file_name = f'Báo cáo Xác nhận nhập rút EIB.xlsx'
  ...
  worksheet.merge_range(f'A{footer_start_row}:D{footer_start_row}','                  XÁC NHẬN CỦA EXIM CHI NHÁNH SÀI GÒN',xacnhancuanganhang_format)
  ...
  worksheet.merge_range(f'G{footer_start_row}:J{footer_start_row}','XÁC NHẬN CỦA CHỨNG KHOÁN PHÚ HƯNG',xacnhancuaphuhung_format)
  ...
  # OCEANBANK
  file_name = f'Báo cáo Xác nhận nhập rút OCB.xlsx'
  writer = pd.ExcelWriter(
    join(dept_folder,folder_name,period,file_name),
    engine='xlsxwriter',
    engine_kwargs={'options':{'nan_inf_to_errors':True}}
  ...
  # Insert image OCB
  worksheet.insert_image('A1','./img/ocb_logo.png')
  ...
  worksheet.merge_range('A5:J6','GIẤY XÁC NHẬN ĐỒNG Ý CHO CÁC KHÁCH HÀNG RÚT TIỀN\nCHUYỂN KHOẢN HOẶC NỘP TIỀN',title_format)
  worksheet.merge_range('H4:J4',f'TP.HCM, Ngày ... Tháng {month} Năm {year}',sub_title_format)
  ...
  worksheet.merge_range(f'A{footer_start_row}:D{footer_start_row}','     XÁC NHẬN CỦA OCB - PHẠM VĂN HAI ',xacnhancuanganhang_format)
  ...
  worksheet.merge_range(f'G{footer_start_row}:J{footer_start_row}','                     XÁC NHẬN CỦA CTY CK PHÚ HƯNG',xacnhancuaphuhung_format)
