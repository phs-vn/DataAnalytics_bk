...
def _get_outlook_files(
  ...
  def f(x):  # Hàm lọc mail có sender là ngân hàng
    try:
      ...
      try:
        ...
        sender = ''
    return re.search('.*@eximbank.*',sender)

  # Tạo folder
  if not os.path.isdir(join(dept_folder,'FileFromBanks',period,'EIB')):
    os.mkdir(join(dept_folder,'FileFromBanks',period,'EIB'))
  ...
  files = os.listdir(join(dept_folder,'FileFromBanks',period,'EIB'))
  for file in files:
    os.remove(join(dept_folder,'FileFromBanks',period,'EIB',file))
  # Get file
  ...
  for attachment in email.Attachments:
    ...
    if check_file_extension:
      attachment.SaveASFile(os.path.join(dept_folder,'FileFromBanks',period,'EIB',attachment.FileName))

  # Return đường dẫn
  file = os.listdir(join(dept_folder,'FileFromBanks',period,'EIB'))[0]  # should only have 1 file
  result = join(dept_folder,'FileFromBanks',period,'EIB',file)
...
def run(
  ...
  # fill sent date
  output = output.join(sent_data['s_balance'].fillna(0),how='outer')
  ...
  file_import.to_pickle(f'{destination}/EIB.pickle')
  ...
  file_name = f"Doi chieu so du EIB truoc khi import {txdate.replace('/','.')}.xlsx"
