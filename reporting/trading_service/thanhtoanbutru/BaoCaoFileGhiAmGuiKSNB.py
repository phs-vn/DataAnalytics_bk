...
def run(  # BC thang
    ...
    LEFT JOIN (
        SELECT 
            [vcf0051].[sub_account], 
            [vcf0051].[bank_name]
        FROM [vcf0051] WHERE [vcf0051].[bank_name] <> '---' AND [vcf0051].[date] <= '{end_date}'
        GROUP BY [sub_account], [bank_name]
    ) [bank_date]
    ON 
        [bank_date].[sub_account] = [money_in_out_transfer].[sub_account]
    WHERE 
