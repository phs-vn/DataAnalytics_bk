...
class DB:
    ...
    def __init__(self,db):
        ...
        self.StoredProcedures = 'sp' + NotRunTables
    ...
    def SendMailAndExec(self):
        ...
        if self.missing_Tables.empty:
            ...
            # Exec stored procedures which have not been run
            today = self.check_time.strftime('%Y-%m-%d')
            cursor = self.conn.cursor()
            cursor.fast_executemany = True
            for sp in self.StoredProcedures:
                EXEC(self.conn,sp,FrDate=today,ToDate=today)
            else:
                cursor.close()
