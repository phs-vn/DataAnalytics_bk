    ...
    def run(
        ...
        for row in range(0,len(rows)):
            ...
            if self.backtest_table.loc[row,'selected_cw'] != 'No CW Available':
                ...
                T_ = self.T
                k_ = self.k
                result_set = object1.hedge(K_,T_,k_,1.0).squeeze(axis=0) # No CW Available -> 100% into underlying stock
        ...
        for cw in selected_cws:
            if cw == 'No CW Available': price_mapper = pd.Series(0,index=self.backtest_table['date'])
            else: price_mapper = self.price_table[cw]
            ...
            self.backtest_table.loc[d_range,'p_hedging_cw'] = self.backtest_table.loc[d_range,'p_hedging_cw'].fillna(method='ffill') # in case of missing data
        ...
        for row in self.backtest_table.index:
            ...
            if row == 0:
                ...
                if current_cw == previous_cw:
                    ...
                    from_due_cw = self.backtest_table.loc[row-1,'n_hedging_cw'] * self.backtest_table.loc[row-1,'p_hedging_cw']
                if row != self.backtest_table.index[-1]:
                    ...
                    from_remaining_cw = self.backtest_table.loc[row,'n_hedging_cw'] * self.backtest_table.loc[row,'p_hedging_cw']
                ...
                from_delta_cw = - self.backtest_table.loc[row,'delta_n_hedging_cw'] * self.backtest_table.loc[row,'p_hedging_cw']
        ...
        if self.backtest_table.iloc[-1,col] > 0: self.status = 'ITM'
        else: self.status = 'OTM'
...
class multiple_backtest:
    ...
    def report(
        ...
        def f(cw):
            ...
            if check_both: final_result = 'Passed'
            else: final_result = 'Failed'
...
def computeMIRR(CF_Series):
    ...
    inflow = CF_Series.loc[CF_Series>=0] ; outflow = CF_Series.loc[CF_Series<0]
    FV_sum = inflow.sum() ; PV_sum = outflow.sum()
    days = len(CF_Series)
