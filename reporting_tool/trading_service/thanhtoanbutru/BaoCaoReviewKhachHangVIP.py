...
def run(
    ...
    vip_phs.drop_duplicates(inplace=True) # do một số KH thay đổi 2 tiểu khoản cùng lúc. VD: '022C002768' ngày 29/10/2021
    ...
    mask = (vip_phs['date_of_change']==vip_phs['effective_date']) | (vip_phs['date_of_approval']==vip_phs['approved_date'])
    vip_phs = vip_phs.loc[mask]
    ...
    for idx in vip_phs.index:
        contract_type = vip_phs.loc[idx,'contract_type']
        if contract_type=='SILV PHS' or contract_type=='GOLD PHS':
            if 1 <= run_time.month < 6:
                vip_phs.loc[idx,'review_date'] = dt.datetime(run_time.year,6,30).strftime("%d/%m/%Y")
            elif 6 <= run_time.month < 12:
                vip_phs.loc[idx, 'review_date'] = dt.datetime(run_time.year,12,31).strftime("%d/%m/%Y")
            else:
                vip_phs.loc[idx,'review_date'] = dt.datetime(run_time.year+1,6,30).strftime("%d/%m/%Y")
        elif contract_type=='VIP Branch':
            if 1 <= run_time.month < 3:
                vip_phs.loc[idx,'review_date'] = dt.datetime(run_time.year,3,31).strftime("%d/%m/%Y")
            elif 3 <= run_time.month < 6:
                vip_phs.loc[idx,'review_date'] =  dt.datetime(run_time.year,6,30).strftime("%d/%m/%Y")
            elif 6 <= run_time.month < 9:
                vip_phs.loc[idx,'review_date'] = dt.datetime(run_time.year,9,30).strftime("%d/%m/%Y")
            else:
                vip_phs.loc[idx,'review_date'] = dt.datetime(run_time.year+1,3,31).strftime("%d/%m/%Y")
    ...
    vip_phs_sheet.write_column(
        'A5',
        np.arange(vip_phs.shape[0])+1,
        num_center_format
    )
    vip_phs_sheet.write_column(
        'B5',
    ...
    vip_phs_sheet.write_column(
        'F5',
        vip_phs['date_of_birth'],
        date_format
    )
    vip_phs_sheet.write_column(
        'G5',
        vip_phs['contract_type'],
        list_customer_vip_col_format
    )
    vip_phs_sheet.write_column(
        'H5',
        vip_phs['effective_date'],
        date_format
    )
    vip_phs_sheet.write_column(
        'I5',
        vip_phs['approved_date'],
        date_format
    )
    vip_phs_sheet.write_column(
        'J5',
    ...
    branch_groupby_sheet.write_column(
        'A2',
        branch_groupby_table.index,
        location_format
    )
    branch_groupby_sheet.write_column(
        'B2',
        branch_groupby_table['GOLD PHS'],
        num_format
    )
    branch_groupby_sheet.write_column(
        'C2',
        branch_groupby_table['SILV PHS'],
        num_format
    )
    branch_groupby_sheet.write_column(
        'D2',
        branch_groupby_table['VIP Branch'],
        num_format
    )
