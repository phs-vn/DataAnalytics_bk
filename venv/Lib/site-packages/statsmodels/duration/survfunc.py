...
def _calc_survfunc_right(time, status, weights=None, entry=None, compress=True,
    ...
    # Size of risk_management set just prior to each event time.
    if weights is None:
...
class SurvfuncRight(object):
    ...
    entry : array_like, optional An array of entry times for handling
        left truncation (the subject is not in the risk_management set on or
    ...
    n_risk : array_like
        The number of subjects at risk_management just before each time value in
    ...
    def summary(self):
        ...
        df["num at risk_management"] = self.n_risk
...
def survdiff(time, status, group, weight_type=None, strata=None,
    ...
    weight_type : str
        The following weight types are implemented:
            ...
            fh : Fleming-Harrington, weights by S^(fh_p),
                 ...
                 always 1.
            gb : Gehan-Breslow, weights by the number at risk_management
    ...
    entry : array_like
        ...
        the risk_management set on or before the entry time.
...
def _survdiff(time, status, group, weight_type, gr, entry=None,
    ...
    # Event count and risk_management set size at each time point, per group and overall.
