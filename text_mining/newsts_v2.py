...
class vsd:
    ...
    def tinTCPH(self, num_hours:int=48) -> pd.DataFrame:
        ...
        while fromtime >= datetime.strptime(bmk_time, '%Y-%m-%d %H:%M:%S'):
            ...
            tags = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions)\
            ...
            for tag_ in tags:
                h3_tag \
                    = WebDriverWait(tag_,wait_time,ignored_exceptions=self.ignored_exceptions)\
                ...
                if any(check):
                    ...
                    sub_url \
                        = WebDriverWait(h3_tag,wait_time,ignored_exceptions=self.ignored_exceptions)\
                        .until(expected_conditions.presence_of_element_located(
                        (By.TAG_NAME, 'a')
                    )).get_attribute('href')
                    ...
                    news_time_ = datetime.strptime(news_time_[-21:],'%d/%m/%Y - %H:%M:%S')
                    ...
                    info_heads \
                        = WebDriverWait(sub_driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                        .until(expected_conditions.presence_of_all_elements_located(
                            (By.XPATH,"//div[substring(@class,string-length(@class)-string-length('item-info')+1)='item-info']")
                    ))
                    ...
                    if len(info_heads) == 0:
                        ...
                        info_heads_text = [head.text[:-1] for head in info_heads]
                        info_tails \
                            = WebDriverWait(sub_driver,wait_time,ignored_exceptions=self.ignored_exceptions)\
                            .until(expected_conditions.presence_of_all_elements_located(
                                (By.XPATH,"//div[substring(@class,string-length(@class)-string-length('item-info-main')+1)='item-info-main']")
                        ))

                        info_tails_text = [tail.text for tail in info_tails]
                        news_dict = {h:t for h,t in zip(info_heads_text, info_tails_text)}
                    ...
                    try:
                        ...
                        content_elem \
                            = WebDriverWait(sub_driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                            .until(expected_conditions.presence_of_element_located(
                                (By.XPATH,"//*[@id='wrapper']/main/div/div/div/div[1]/div[2]/div/div/p[2]")
                        ))
                    ...
                    exchange_deposit = ''
                    try:
                        noidung = np.array(content_split)[np.array(mask_date)][0]
                    except IndexError:
                        noidung = ''
                    ...
                    if noidung == '':
                        try:
                            ...
                            content_elem \
                                = WebDriverWait(sub_driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                                .until(expected_conditions.presence_of_element_located(
                                    (By.XPATH,'//*[@id="wrapper"]/main/div/div/div/div[1]/div[2]/div/div/p[2]/b')
                            ))
                            noidung = content_elem.text
                            content_elem \
                                = WebDriverWait(sub_driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                                .until(expected_conditions.presence_of_element_located(
                                    (By.XPATH,'//*[@id="wrapper"]/main/div/div/div/div[1]/div[2]/div/div/div[14]')
                            ))
            ...
            time.sleep(2)
            nextpage_button \
                = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions)\
                .until(expected_conditions.presence_of_all_elements_located(
                    (By.XPATH,"//button[substring(@onclick,1,string-length('changePage'))='changePage']")))[-2]
            ...
            fromtime = datetime.strptime(fromtime[-21:],'%d/%m/%Y - %H:%M:%S')
    ...
    def tinTVLK(self, num_hours:int=48) -> pd.DataFrame:
        ...
        while fromtime >= datetime.strptime(bmk_time, '%Y-%m-%d %H:%M:%S'):
            ...
            for tag_ in tags:
                h3_tag = WebDriverWait(tag_,wait_time,ignored_exceptions=self.ignored_exceptions)\
                    .until(expected_conditions.presence_of_element_located(
                        (By.TAG_NAME, 'h3')
                ))
                ...
                if all(check_1) and any(check_2):
                    ...
                    info_heads = sub_driver.find_elements_by_xpath(
                        "//div[substring(@class,string-length(@class)-string-length('item-info')+1)='item-info']"
                    )
                    ...
                    info_tails \
                        ...
                        "//div[substring(@class,string-length(@class)-string-length('item-info-main')+1)='item-info-main']"
                    )
                    info_tails_text = [tail.text for tail in info_tails]

                    news_dict = {h: t for h, t in zip(info_heads_text, info_tails_text)}

                    news_head = [t for t in info_heads_text if t.startswith('Ngày giao dịch')][0]
                    try:
            ...
            if not output_table['Tiêu đề'].empty:
...
class hnx:
    ...
    def tintuso(self, num_hours:int=48):
        ...
        while from_time >= datetime.strptime(bmk_time, '%Y-%m-%d %H:%M:%S'):
            ...
            titles_inpage = []
            title_elems = []
            while len(tickers_inpage) == 0 or len(titles_inpage) == 0 or len(title_elems) == 0:
                try:
                    ticker_elems \
                        = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                        .until(expected_conditions.presence_of_all_elements_located(
            ...
            for ticker_title in ticker_title_inpage:
                ...
                titles += [driver.find_elements_by_xpath(f"//*[@id='_tableDatas']/tbody/tr[{title_no}]/td[4]")[0].text]
                times += [driver.find_elements_by_xpath(f"//*[@id='_tableDatas']/tbody/tr[{title_no}]/td[2]")[0].text]
                tickers += [driver.find_elements_by_xpath(f"//*[@id='_tableDatas']/tbody/tr[{title_no}]/td[3]")[0].text]
                ...
                popup_content = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions)\
                    .until(expected_conditions.presence_of_element_located(
...
class hose:
    ...
    def tinTCNY(self, num_hours:int=48):
        ...
        while from_time >= datetime.strptime(bmk_time, '%Y-%m-%d %H:%M:%S'):
            ...
            headline_text = []
            headline_tags = []
            while len(headline_tags) == 0:
                try:
                    print('vi tri 1')
                    headline_tags \
                        = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                        .until(expected_conditions.presence_of_all_elements_located((By.XPATH, '*//td[3]//*')))[1:]
                    headline_text = [t.text for t in headline_tags]
                except (Exception,):
                    pass

            time_text = []
            while len(time_text) == 0:
                try:
                    print('vi tri 2')
                    time_tags \
                        = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
            ...
            for num in range(len(headline_text)):
                ...
                if any(check_1) and all(check_2):
                    ...
                    content = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                    ...
                    pdf_elements = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                        .until(expected_conditions.presence_of_all_elements_located(
    ...
    def tinCW(self, num_hours:int=48):
        ...
        while from_time >= datetime.strptime(bmk_time,'%Y-%m-%d %H:%M:%S'):
            ...
            time_tags = []
            while len(time_tags) == 0:
                time_tags = WebDriverWait(driver,wait_time,ignored_exceptions=self.ignored_exceptions)\
                                .until(expected_conditions.presence_of_all_elements_located(
                    (By.XPATH,'*//td[2]')
                ))[2:]
            ...
            for num in range(len(headline_text)):
                ...
                if any(check):
                    ...
                    content = WebDriverWait(sub_driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                        .until(expected_conditions.presence_of_element_located(
                            (By.XPATH, '//*[@id="body"]//*//div[2]/p')
                    )).text

                    pdf_elements = WebDriverWait(sub_driver,wait_time,ignored_exceptions=self.ignored_exceptions) \
                        .until(expected_conditions.presence_of_all_elements_located(
                            (By.XPATH, '*//td[2]/a')
                    ))
