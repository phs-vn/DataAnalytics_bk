...
def __CheckAbnormal__(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        abnormalTable = fullTable.loc[fullTable['check'] == 'Bất thường']

        if not abnormalTable.empty:
            tmp = pd.DataFrame(abnormalTable.apply(lambda x: getBranchBroker(x['account_code'], dt.datetime.now()), axis=1).to_list())
            abnormalTable[['branch_name', 'broker_name']] = tmp
            abnormalTable = abnormalTable.rename(columns={
                'account_code': 'Số Tài Khoản',
                'sub_account': 'Số Tiểu Khoản',
                'customer_name': 'Tên Khách Hàng',
                'value_t2': 'Giá trị tiền bán T-2',
                'fee_t2': 'Phí bán T-2',
                'sell_tax_t2': 'Thuế bán T-2',
                'dividend_tax_t2': 'Thuế cổ tức T-2',
                'value_t1': 'Giá trị tiền bán T-1',
                'fee_t1': 'Phí bán T-1',
                'sell_tax_t1': 'Thuế bán T-1',
                'dividend_tax_t1': 'Thuế cổ tức T-1',
                'value_t0': 'Giá trị tiền bán T0',
                'fee_t0': 'Phí bán T0',
                'sell_tax_t0': 'Thuế bán T0',
                'dividend_tax_t0': 'Thuế cổ tức T0',
                'payback_uttb_t0': 'Tiền Hoàn trả UTTB T0',
                'available_to_advance': 'Tổng giá trị tiền bán có thể ứng',
                'advanced_amount_t2': 'Số tiền UTTB KH đã nhận của ngày T-2',
                'advanced_fee_t2': 'Phí UTTB của ngày T-2',
                'advanced_amount_t1': 'Số tiền UTTB KH đã nhận của ngày T-1',
                'advanced_fee_t1': 'Phí UTTB của ngày T-1',
                'advanced_amount_t0': 'Số tiền UTTB KH đã nhận của ngày T0 và T-1',
                'advanced_fee_t0': 'Phí UTTB của ngày T0 và T-1',
                'remaining_advance': 'Tổng tiền còn có thể ứng',
                'check': 'Bất Thường',
                'branch_name': 'Chi nhánh quản lý',
                'broker_name': 'Nhân viên quản lý tài khoản'
            })
            ...
            mail.Subject = f"Dữ liệu bất thường Báo cáo Đối chiếu UTTB {dt.datetime.now().strftime('%d.%m.%Y')}"
...
def run(
    ...
    check_1 = table['payback_uttb_t0'] > (able_to_advance_t1 + able_to_advance_t2)
