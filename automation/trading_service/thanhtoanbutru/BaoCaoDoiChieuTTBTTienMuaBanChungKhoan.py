...
def __CheckAbnormal__(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        abnormalTable = fullTable.loc[
            (fullTable['diff_value'] != 0) | (fullTable['diff_fee'] != 0) | (fullTable['diff_tax'] != 0)
        ]
        if not abnormalTable.empty:
            abnormalTable = abnormalTable.reset_index()
            tmp = pd.DataFrame(abnormalTable.apply(lambda x: getBranchBroker(x['account_code'],x['pay_date']),axis=1).to_list())
            abnormalTable[['branch_name', 'broker_name']] = tmp
            abnormalTable = abnormalTable.rename(columns={
                'trade_date': 'Ngày giao dịch',
                'pay_date': 'Ngày thanh toán tiền',
                'account_code': 'Số Tài Khoản',
                'sub_account': 'Số Tiểu Khoản',
                'customer_name': 'Tên Khách Hàng',
                'type_of_order': 'Loại lệnh',
                'value_order': 'Giá trị khớp - Theo KQ khớp lệnh',
                'fee_order': 'Phí - Theo KQ khớp lệnh',
                'tax_order': 'Thuế - Theo KQ khớp lệnh',
                'value_cash': ' Giá trị khớp - Theo GD tiền',
                'fee_cash': 'Phí - Theo GD tiền',
                'tax_cash': 'Thuế - Theo GD tiền',
                'diff_value': 'Giá trị khớp - Lệch',
                'diff_fee': 'Phí - Lệch',
                'diff_tax': 'Thuế - Lệch',
            ...
            mail.Subject = f"Dữ liệu bất thường Báo cáo Đối chiếu TTBT tiền mua bán chứng khoán {dt.datetime.now().strftime('%d.%m.%Y')}"
...
def run(
    ...
    table.rename({'date': 'pay_date', 'value': 'value_order', 'fee': 'fee_order', 'tax': 'tax_order'}, axis=1,
                 inplace=True)
