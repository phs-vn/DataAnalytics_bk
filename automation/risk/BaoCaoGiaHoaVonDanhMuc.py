...
def run(  # chạy daily sau batch cuối ngày
    ...
    table = pd.read_sql(
        ...
        LEFT JOIN (
            ...
            WHERE [margin_outstanding].[type] IN (N'Margin',N'Trả chậm',N'Bảo lãnh') AND [margin_outstanding].[date] = '2022-02-17'
            GROUP BY [margin_outstanding].[account_code]
            ) [outstanding] 
            ON [outstanding].[account_code] = [t].[account_code]
        LEFT JOIN [rmr0062] ON [rmr0062].[account_code] = [t].[account_code] AND [rmr0062].[date] = '2022-02-17' AND [rmr0062].[loan_type] = 1
        LEFT JOIN [vpr0109] ON [vpr0109].[ticker_code] = [t].[ticker] AND [vpr0109].[date] = '2022-02-17'AND [vpr0109].[room_code] LIKE 'TC01%'
